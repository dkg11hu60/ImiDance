...
export function LocationManagement() {
  ...
  const handleAddLocation = async (e: React.FormEvent) => {
    ...
    if (error) {
      alert(`Hiba a helyszín mentésekor: ${error.message}`)
    } else {
      setName('')
      setAddress('')
      setMapsUrl('')
  ...
  if (loading) {
    return <div className="p-4 text-sm text-zinc-500">Helyszínek betöltése...</div>
  }

  if (error) {
    return (
      <div className="p-4 text-sm text-red-600 bg-red-50 rounded-lg border border-red-200">
        Hiba a helyszínek betöltésekor: {error}
      </div>
    )
  }
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-zinc-500">
            A táncórák és események helyszíneinek kezelése.
          </p>
        </div>
        <button
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {locations.length === 0 ? (
          <p className="text-sm text-zinc-500 italic col-span-2">
            Még nincs egyetlen helyszín sem rögzítve.
          </p>
        ) : (
            ...
            >
              ...
              {location.maps_url && (
                <div>
                  ...
                  >
                    Megnyitás Google Térképen &rarr;
                  </a>
                </div>
              )}
            </div>
            ...
            <form onSubmit={handleAddLocation} className="space-y-4">
              ...
              <div className="space-y-1">
                <div className="flex items-center justify-between">
                  ...
                  >
                    {showHelper ? 'Súgó elrejtése' : 'Hogyan kell kimásolni?'}
                  </button>
                </div>
                <input
                  type="url"
                ...
                {showHelper && (
                  <div className="mt-2 p-3 bg-indigo-50/80 border border-indigo-100 rounded-lg text-xs space-y-2 text-zinc-700">
                    <p className="font-semibold text-indigo-900">
                      Útmutató a hivatkozás kimásolásához:
                    </p>
                    <ol className="list-decimal list-inside space-y-1 text-zinc-600">
                      ...
                      <li>Keresd meg és kattints a kiválasztott helyszínre.</li>
                      <li>
                        Kattints a **Megosztás** (vagy *Share*) gombra.
                      </li>
                      <li>
                        A felugró ablakban kattints a **Link másolása** (vagy *Copy link*) gombra.
                      </li>
                      <li>Illeszd be a kimásolt hivatkozást ide a fenti mezőbe!</li>
                    </ol>
                  </div>
                )}
              </div>
              ...
              <div className="flex justify-end space-x-3 pt-3 border-t border-zinc-100">
                ...
                >
                  {submitting ? 'Mentés...' : 'Helyszín mentése'}
                </button>
              </div>
            </form>
          </div>
