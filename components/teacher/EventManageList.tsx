...
const PLACEHOLDER_DOMAIN = '@imisdance.local'
const isRealEmail = (email: any): boolean => {
  const e = (email || '').toString().trim().toLowerCase()
  return e.length > 0 && e.includes('@') && !e.endsWith(PLACEHOLDER_DOMAIN)
}
...
export function EventManageList() {
  ...
  const [noEmail, setNoEmail] = useState<{ name: string }[]>([])
  ...
  const fmtDate = (ts: any): string => {
    ...
    const d = new Date(ts)
    if (isNaN(d.getTime())) return ts.toString().split('T')[0]
    return d.toLocaleDateString('hu-HU', { year: 'numeric', month: '2-digit', day: '2-digit' })
  }
  const fmtTime = (t: any): string => (t ? t.toString().slice(0, 5) : '')
  const locName = (ev: any): string => locationMap.get(ev.location_id) || 'Helyszín nélkül'

  async function openModal(ev: any) {
    setModalEvent(ev)
    setResult(null)
    setAck(false)
    setPrepping(true)
    ...
    const { data: atts } = await supabase
      .from('attendances').select('profile_id').eq('event_id', ev.id)
    const ids = Array.from(new Set((atts || []).map((a: any) => a.profile_id).filter(Boolean)))
    ...
    if (ids.length > 0) {
      ...
      for (const p of profs || []) {
        ...
        if (isRealEmail(p.email)) recs.push({ name, email: p.email.trim() })
        else none.push({ name })
      }
    }
    setRecipients(recs)
  ...
  async function finalize() {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        setResult(json)
        setEvents(prev => prev.map(e => e.id === modalEvent.id ? { ...e, is_active: false } : e))
      }
    } catch (e: any) {
      setResult({ error: (e?.message || 'Hálózati hiba').toString() })
    } finally {
      setSending(false)
    }
  }
      ...
      <div className="space-y-3">
        {events.map(ev => {
          ...
          return (
            <div
              key={ev.id}
              className={`rounded-2xl border p-3.5 bg-white ${inactive ? 'border-red-200 overflow-hidden' : 'border-zinc-200'}`}
            >
              {inactive && (
                <div className="mb-2.5 -mx-3.5 -mt-3.5 px-3.5 py-1 bg-red-600">
                  <span className="text-white font-extrabold text-xs tracking-wider uppercase">TÖRÖLVE</span>
                </div>
              )}
              <div className="flex items-center justify-between gap-3">
                <div className={`min-w-0 flex-1 ${inactive ? 'line-through opacity-60' : ''}`}>
                  <p className="font-bold text-sm sm:text-base text-zinc-900 leading-snug">{fmtDate(ev.event_date)}</p>
                  <p className="text-xs sm:text-sm text-zinc-600">
                    {fmtTime(ev.start_time)}{ev.end_time ? `–${fmtTime(ev.end_time)}` : ''} · {locName(ev)}
                  </p>
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-2xl max-w-lg w-full max-h-[90vh] overflow-y-auto p-6 space-y-4">
            ...
            <div className="p-3 bg-red-50 border border-red-200 rounded-lg text-sm text-red-800">
              Ez a művelet <strong>nem visszavonható</strong>. Az inaktiválás után az alkalom
              „TÖRÖLVE” állapotba kerül, és nem lehet rá többé jelentkezni.
            </div>

            <div className="text-sm">
              <p className="font-semibold">{fmtDate(modalEvent.event_date)} · {locName(modalEvent)}</p>
              <p className="text-zinc-600">
                {fmtTime(modalEvent.start_time)}{modalEvent.end_time ? `–${fmtTime(modalEvent.end_time)}` : ''}
              </p>
            </div>
            ...
            {prepping ? (
              ...
              <>
                <div className="space-y-2">
                  <p className="text-sm font-semibold">Értesítendő jelentkezők ({recipients.length} fő)</p>
                  {recipients.length > 0 ? (
                    <ul className="text-xs text-zinc-700 max-h-32 overflow-y-auto border rounded p-2 space-y-0.5">
                      {recipients.map((r, i) => (
                        <li key={i}>{r.name} <span className="text-zinc-400">&lt;{r.email}&gt;</span></li>
                      ))}
                    </ul>
                  ) : (
                  ...
                  {noEmail.length > 0 && (
                    <div className="text-xs text-amber-700 bg-amber-50 border border-amber-200 rounded p-2">
                      <p className="font-semibold">Nincs e-mail címük ({noEmail.length} fő) — kézzel értesítendő:</p>
                ...
                <label className="flex items-start gap-2 text-sm">
                  ...
                  <span>Tudomásul veszem, hogy az inaktiválás <strong>végleges és nem visszavonható</strong>.</span>
                </label>
                  ...
                  result.error ? (
                    ...
                    <div className="text-sm text-green-800 bg-green-50 border border-green-200 rounded p-2 space-y-1">
                      ...
                      {result.skipped_no_email?.length > 0 && (
                        <p>E-mail nélkül (kézzel): {result.skipped_no_email.map((s: any) => s.name).join(', ')}</p>
                      )}
                      {result.failed?.length > 0 && (
                        <div className="text-red-700 bg-red-50 p-2 rounded text-xs space-y-1">
                          ...
                          {result.failed.map((f: any, i: number) => (
                            <p key={i}>• {f.email}: <span className="font-mono text-[11px]">{f.error}</span></p>
                          ))}
                        </div>
                      )}
                ...
                <div className="flex gap-2 pt-2">
                  {(!result || result.error) ? (
                    <>
                      ...
                      <button onClick={closeModal} className="px-4 py-2 border rounded-lg text-sm">Mégse</button>
                    </>
                  ) : (
                    <button onClick={closeModal} className="flex-1 py-2 border rounded-lg">Bezárás</button>
                  )}
                </div>
              </>
            )}
