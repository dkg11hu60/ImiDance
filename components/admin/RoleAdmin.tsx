...
export function RoleAdmin() {
  ...
  async function load() {
    ...
    try {
      ...
      if (objectsRes.error) throw new Error(`App Objects: ${objectsRes.error.message}`)
      if (matrixRes.error) throw new Error(`Object Roles: ${matrixRes.error.message}`)
      ...
      setMsg({ type: 'err', text: 'Nem sikerült betölteni az adatokat: ' + err.message })
    } finally {
      setLoading(false)
    }
  }
  ...
  async function deleteRole(role: Role) {
    ...
    if (!confirm(`Biztosan törlöd a(z) "${role.label}" szerepet?`)) {
      return
    }
  ...
  if (loading) return <div className="text-zinc-500 p-6 animate-pulse">Szerepek és objektumok betöltése...</div>
    ...
    <div className="space-y-8">
      <div>
        ...
        <p className="text-xs text-zinc-500">
          A rács soronként egy objektum, oszloponként egy szerep. A pipa jelöli az engedélyezett kapcsolatot.
        </p>
      </div>
      ...
      {objects.length === 0 ? (
        <div className="p-4 bg-amber-50 border border-amber-200 rounded-xl text-amber-800 text-sm">
          Nincsenek objektumok az <strong>app_objects</strong> táblában.
        </div>
      ) : (
        <div className="overflow-x-auto">
          <table className="text-sm border-collapse w-full">
            <thead>
              <tr>
                <th className="text-left py-2 pr-4 border-b text-zinc-600">Objektum</th>
                {roles.map(role => (
                  <th key={role.key} className="py-2 px-3 border-b text-center whitespace-nowrap">
                    <div className="font-semibold text-zinc-900">{role.label}</div>
            ...
            <tbody>
              {objects.map(obj => (
                <tr key={obj.key} className="border-b hover:bg-zinc-50/50">
                    ...
                    return (
                      <td key={role.key} className="py-2 px-3 text-center">
                        <input
                          ...
                          className="w-4 h-4 accent-indigo-600 disabled:opacity-60 cursor-pointer disabled:cursor-not-allowed"
                        />
                      </td>
                    )
                  })}
        ...
        <ul className="space-y-1">
          {roles.map(role => (
            <li key={role.key} className="flex items-center gap-3 text-sm">
              ...
              <button onClick={() => renameRole(role)} className="text-xs text-indigo-600 hover:underline">Átnevez</button>
              {role.key !== PROTECTED_ROLE && (
                <button onClick={() => deleteRole(role)} className="text-xs text-red-600 hover:underline">Töröl</button>
              )}
            </li>
          ))}
        </ul>
