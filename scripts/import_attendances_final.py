...
def main():
    ...
    # 2. Dátumok párosítása az 1. sorból (index 0)
    excel_event_mapping = {} # { col_index: event_id }
    if len(df) > 0:
        row_zero = df.iloc[0]
        for c in range(1, len(df.columns)):
            ...
            if date_str:
                ...
                if matching_event:
                    excel_event_mapping[c] = matching_event["id"]

    print(f"Párosított oszlopok száma: {len(excel_event_mapping)}")
    ...
    # 3. Jelentkezések összegyűjtése a 2. sortól kezdve
    attendance_records = []
    not_found_names = set()
    ...
    for row_idx in range(1, len(df)):
        ...
        if not profile_id:
            not_found_names.add(name_str)
            continue

        for col_idx, event_id in excel_event_mapping.items():
    ...
    if not_found_names:
        print(f"Figyelem: {len(not_found_names)} név nem található a profilok között: {list(not_found_names)[:5]}...")
