-- Migration: 002_monthly_passes.sql
-- Description: Creates monthly_passes table and automated triggers for bérlet system
...
BEGIN
  IF TG_OP = 'INSERT' THEN
    -- Amikor bérletet vesznek: az adott év/hónap összes meglévő jelentkezését 'paid = true'-ra állítjuk
  ...
  ELSIF TG_OP = 'DELETE' THEN
    -- Amikor a bérletet kiveszik: az adott év/hónap összes jelentkezésénél 'paid = false'-ra állítjuk vissza
    UPDATE public.attendances a
    SET paid = false
    FROM public.events e
    WHERE a.event_id = e.id
